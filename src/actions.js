...
export const actions = {
    async updatePageContext() {
        ...
        if (currentPage) {
            ...
            if ('page' in currentPage && currentPage.page) {
                const pageRef = currentPage.page;
            ...
            const pageName = (currentPage.originalName || currentPage.name || "Untitled");
    ...
    async sendMsg() {
        if (!state.tempInput.trim() || state.isBusy || !state.currentPageUuid)
            return;
    ...
    async applyToPage() {
        if (state.isBusy || !state.currentPageUuid)
            return;
        const targetUuid = state.currentPageUuid;
        state.processingPageUuid = targetUuid;
